...
public:
    ...
    vector<int> leftmostBuildingQueries(vector<int>& heights, vector<vector<int>>& queries) {
        ...
        //     int idx_0 = min(query[0], query[1]), idx_1 = max(query[0], query[1]);

        //     if (idx_0 == idx_1 || heights[idx_0] < heights[idx_1]) { // 特殊情况
        //         ans[i] = idx_1;
        //         continue;
        //     }

        //     idx[idx_1].emplace_back(heights[idx_0], i); // 左边的数不小于右边的数
        // }

        // // 模拟单调栈
        // vector<pair<int, int>> stk;
        // stk.emplace_back(inf, -1);
        // for (int i = n - 1; i >= 0; --i) {
        //     // 找到最左边的更高的 h，再通过二分查找
        ...
        //     // 二分
        //     for (auto& v : idx[i]) {
        //         auto iter = lower_bound(stk.begin(), stk.end(), make_pair(v.first, inf), greater<pair<int, int>>());
        ...
        vector<vector<pair<int, int>>> l(m);
        vector<int> ans(n, -1);
